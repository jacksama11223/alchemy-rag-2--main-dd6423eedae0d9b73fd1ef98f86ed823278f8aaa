    ...
    return (
        <>
            ...
            <div className="absolute left-6 top-24 bottom-24 z-[45] flex gap-4 pointer-events-none">
                <div className="w-16 bg-[#0f172a]/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl shadow-black/50 flex flex-col items-center py-4 gap-4 pointer-events-auto overflow-hidden">
                    <Reorder.Group axis="y" values={tabs} onReorder={setTabs} className="flex flex-col gap-4 w-full items-center">
                        {tabs.map(tab => (
                            <Reorder.Item
                                key={tab.id}
                                value={tab}
                                drag="x" // Allow dragging in both x and y, but Reorder handles y
                                dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }}
                                dragElastic={1} // Allow pulling outside constraints
