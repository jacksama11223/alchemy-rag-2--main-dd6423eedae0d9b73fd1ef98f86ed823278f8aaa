...
interface YoutubeHighlighterFlashcardProps {
    videoData: any;
}

const YoutubeHighlighterFlashcard: React.FC<YoutubeHighlighterFlashcardProps> = ({ videoData }) => {
    ...
    const handleSaveFlashcard = async () => {
        ...
        try {
            const response = await fetch('/api/flashcardsets', {
                ...
                body: JSON.stringify({
                    title: `Flashcard từ Video: ${videoData.title}`,
                    description: 'Tạo tự động từ Trích xuất YouTube',
                    sourceNoteId: videoData._id,
                    cards: [
                        {
                    ...
                    tags: ['youtube-extract', 'auto-generated']
                })
            });
    ...
    return (
        <div className="relative h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50 rounded-xl border border-slate-200 p-6">
                ...
                <div 
                    ref={contentRef} 
                    className="prose prose-slate max-w-none text-slate-700 leading-relaxed whitespace-pre-wrap pb-24"
                >
                    {videoData.subtitles || 'Không có dữ liệu phụ đề.'}
                </div>
            </div>
...
export default YoutubeHighlighterFlashcard;
