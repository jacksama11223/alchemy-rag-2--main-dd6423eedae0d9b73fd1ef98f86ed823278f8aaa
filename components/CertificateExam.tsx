...
import { KnowledgeNode, LeaderboardEntry, RankProfile } from '../types';
import { useGamification } from '../contexts/GamificationContext';
import { getLeaderboardData } from '../services/mockBackend';
        ...
        <div className="relative flex h-auto min-h-screen w-full flex-col bg-[#091428] overflow-x-hidden font-display text-[#F0E6D2] selection:bg-[#C8AA6E] selection:text-[#091428]">
            ...
            <header className="flex items-center justify-between px-8 py-5 border-b border-[#785A28] bg-[#010A13]/95 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <span className="material-symbols-outlined text-4xl text-[#C8AA6E] animate-pulse">swords</span>
                    <div>
                        <h2 className="text-2xl font-black uppercase tracking-[0.1em] text-[#F0E6D2]">Đấu Trường Chân Lý</h2>
                        <p className="text-xs text-[#A09B8C] tracking-wide">Ranked Season 1</p>
                    </div>
                </div>
                <button onClick={onBack} className="flex items-center gap-2 text-[#C8AA6E] hover:text-[#F0E6D2] transition-colors border border-[#785A28] px-4 py-2 rounded uppercase text-xs font-bold bg-[#1E2328] hover:bg-[#32281E]">
                    <span className="material-symbols-outlined text-sm">arrow_back</span> Thoát
                </button>
            </header>

            <main className="flex-grow p-8 flex gap-8 max-w-[1600px] mx-auto w-full">
                ...
                <div className="w-[350px] shrink-0 flex flex-col gap-6">
                    ...
                    <div className="hextech-bg border border-[#3C3C41] rounded-xl p-4">
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto scroll-gold pr-1">
                ...
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="bg-[#010A13] border border-[#3C3C41] rounded-xl flex-1 overflow-hidden flex flex-col shadow-2xl">
                        ...
                        <div className="overflow-y-auto flex-1 scroll-gold">
                            {loading ? (
                                ...
                                leaderboard.map((entry, idx) => (
                                    <div key={entry.id} className={`grid grid-cols-12 items-center px-6 py-3 border-b border-[#1E2328] hover:bg-[#1E2328] transition-colors group cursor-pointer ${entry.id === 'current_user' ? 'bg-[#C8AA6E]/10 border-[#C8AA6E]/30' : ''}`}>
                                        ...
                                        <div className="col-span-6 flex items-center gap-4 pl-4">
                                            ...
                                            <span className={`font-bold text-sm ${entry.id === 'current_user' ? 'text-[#C8AA6E]' : 'text-[#F0E6D2]'} group-hover:text-white transition-colors`}>{entry.name}</span>
                                        </div>
