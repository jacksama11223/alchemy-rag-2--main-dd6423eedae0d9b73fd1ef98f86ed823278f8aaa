...
import { getRankProfile, updateRankPoints, checkInUser, getCurrentUser, addXPToBackend } from '../services/mockBackend';
...
interface GamificationContextType {
    ...
    updateRank: (points: number) => Promise<void>; // Wrapper for backend call
}
...
export const GamificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // 1. Load from LocalStorage/Session on Mount
    useEffect(() => {
        const currentUser = getCurrentUser();
        // Fallback to local storage if user has data there
        const saved = localStorage.getItem('learnai_gamification');
        let initialProgress = DEFAULT_PROGRESS;

        if (currentUser) {
            // Prioritize server data found in session
            initialProgress = {
                ...DEFAULT_PROGRESS,
                xp: currentUser.xp || 0,
                level: currentUser.level || 1,
                streak: currentUser.streak || 0,
                // Ensure we have a valid date string or default to epoch
                lastCheckIn: currentUser.lastCheckIn ? new Date(currentUser.lastCheckIn).toISOString() : new Date(0).toISOString()
            };
        } else if (saved) {
            try {
                const parsed = JSON.parse(saved);
                initialProgress = { ...DEFAULT_PROGRESS, ...parsed };
            } catch(e) {}
        }
        
        // Ensure achievements exist
        if (!initialProgress.achievements || initialProgress.achievements.length === 0) {
            initialProgress.achievements = [...INITIAL_ACHIEVEMENTS];
        }
    ...
    // 2. Save to LocalStorage on Change (Backup)
    useEffect(() => {
    ...
    const addXP = async (amount: number, reason: string) => {
        const currentUser = getCurrentUser();
        if (currentUser) {
            const result = await addXPToBackend(amount, reason);
            if (result) {
                setProgress(prev => {
    ...
    const updateRank = async (points: number) => {
        try {
            const currentUser = getCurrentUser();
            let result;
            if (currentUser) {
                result = await updateRankPoints(points, points > 0 ? 'Victory' : 'Defeat');
            } else {
                result = { lp: 0, rankTier: 'Iron', rankDivision: 'IV' }; // Fallback
            }
            
            setRankProfile(prev => ({ ...prev, ...result }));
            
            // Simplified promotion/demotion check for notification
            if (points > 0 && result.lp === 0) {
                showNotification("THĂNG HẠNG! " + result.rankTier + " " + result.rankDivision, points, 'lp');
            } else if (points < 0 && result.lp === 0) {
                showNotification("Rớt hạng xuống " + result.rankTier + " " + result.rankDivision, points, 'demote');
            } else {
    ...
    // 5. Daily Check-in Logic (UPDATED TO USE API)
    const checkIn = async () => {
        ...
        if (user) {
            ...
            if (result.success) {
                ...
                showNotification(result.message, 50);
                return;
            }
        }

        // Fallback Local Logic (Offline or Guest)
        ...
        showNotification(`Điểm danh ngày thứ ${newStreak}! (Offline Mode)`, 50);
    };
