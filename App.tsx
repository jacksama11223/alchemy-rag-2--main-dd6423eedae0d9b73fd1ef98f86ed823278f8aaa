...
function App() {
    ...
    if (user) {
        ...
        if (user.isAdmin) {
             ...
             import('./services/mockBackend').then(module => {
                 module.getClustersFromBackend().then(clusters => setUserClusters(clusters));
             });
        }
    }
  ...
  const handleAddNodesAndCluster = useCallback((nodes: KnowledgeNode[], cluster: any) => { 
      setUserNodes(prev => [...prev, ...nodes]); 
      setUserClusters(prev => [...prev, cluster]);
      addXP(nodes.length * 20 + 100, "Kiến trúc sư tri thức - Lộ trình Học tập"); 
      logAction('create_content', 'Graph', `Created Roadmap cluster and ${nodes.length} nodes`); 
  }, [setUserNodes, setUserClusters, addXP, logAction]);
  ...
  return (
    ...
    />
      ...
      <main className="flex-grow flex flex-col relative z-0" style={mainStyle}>
        ...
        {view === 'alchemy' && <Alchemy onBack={handleGoBackToDashboard} onShowAbout={handleShowAbout} onLogout={handleLogout} onShowFAQ={handleShowFAQ} onShowAccount={handleShowAccount} onAddNode={handleAddNode} onAddNodes={handleAddNodes} onAddNodesAndCluster={handleAddNodesAndCluster} onGoToGraph={() => handleFeatureSelect('explore-graph')} userNodes={userNodes} onUpdateNode={handleUpdateNode} onNavigateToDistill={() => handleFeatureSelect('video')} intent={alchemyIntent} onGoToFeatures={handleGoToFeatures} onGoToBattle={() => setView('battle')} onToggleTodo={toggleTodoPanel} />}
        {view === 'explore-graph' && <ExploreGraph onBack={handleGoBackToDashboard} onShowAbout={handleShowAbout} onSearch={handleExploreSearch} onCategory={handleExploreCategory} onLogout={handleLogout} onShowFAQ={handleShowFAQ} onShowAccount={handleShowAccount} userNodes={userNodes} userClusters={userClusters} onNodeClick={handleNodeClick} onOpenNode={handleNodeOpen} onStartPlaylist={handleStartPlaylist} onMergeNodes={handleMergeNodes} onDeleteNodes={handleDeleteNodes} activeFilter={activeTagFilter} onClearFilter={() => setActiveTagFilter(null)} onExpandNode={handleExpandNode} onAskTutor={handleAskTutor} focusedNodeId={focusedNodeId} onNavigateToAlchemy={handleNavigateToAlchemy} onNavigateToFeature={handleFeatureSelect} quests={quests} userXP={progress.xp} userLevel={progress.level} currentAchievement={activeAchievement} onClaimReward={handleClaimQuest} onCloseAchievement={() => setActiveAchievement(null)} onGainXP={handleGainXP} onRegisterQuest={handleRegisterQuest} onAddTask={handleAddTask} onGoToFeatures={handleGoToFeatures} onToggleTodo={toggleTodoPanel} intent={alchemyIntent} onClearIntent={() => setAlchemyIntent(null)} />}
